 ...
 public function laporan()
    ...
    public function simpan_pesanan_temp()
    {
        $masukan = $this->input->post();
            ...
            $get_menu = $this->db->get_where('master_bahan_baku', array('kode_bahan_baku' => $kode_produk));
            ...
            if (count($hasil_cek_menu) < 1) {
                ...
                $this->db->update('opsi_transaksi_penjualan_temp', $updatemenu, array(
                    'kode_penjualan' => $masukan['kode_penjualan'],'kode_menu'=>$kode_produk));
            }
    ...
    public function simpan_pembayaran()
    {
        ...
        foreach ($hasil_pesanan as $daftar) {
            ...
            $this->db->update('master_bahan_baku',$updatestok,array('kode_bahan_baku'=>$daftar->kode_menu));
        }
        $this->db->group_by('kode_meja');
    ...
    public function get_total_temp()
    {
        ...
        $get_total = $this->db->get_where('opsi_transaksi_penjualan_temp', array('kode_penjualan' =>
                $kode_penjualan));
        $hasil = $get_total->row();
    ...
    public function cetak_pembayaran()
    {
        ...
        foreach ($hasil_menu as $daftar) {
            ...
            $printTestText .= bill_php_Left($daftar->nama_menu, 45)."\n". bill_php_Left($daftar->
                harga_satuan, 13) . bill_php_right($daftar->jumlah, 6) . bill_php_right($daftar->
                diskon_item, 9) . "%" . bill_php_right($daftar->subtotal, 16) . "\n";
        }
        ...
        $printTestText .= "Grand Total	: Rp" . bill_php_right(format_rupiah_norp($hasil_penjualan->
            grand_total),25) . "\n";
        $printTestText .= "Bayar    	: Rp" . bill_php_right(format_rupiah_norp($hasil_penjualan->bayar),25) . "\n";
        if ($jenis_bayar == 'kredit') {
            $hasil = "Sisa   ";
        } else {
            $hasil = "Kembali";
        }
        $printTestText .= "$hasil  	: Rp" . bill_php_right(format_rupiah_norp(abs($hasil_penjualan->kembalian)),25) .
            "\n";
        $printTestText .= "---------------------------------------------\n";
        $printTestText .= "                   Terima Kasih             \n";
        $printTestText .= "              Barang yang sudah dibeli      \n";
