                                            ...
                                            if (getenv('HTTP_FORWARDED_FOR'))
                                                ...
                                                if (getenv('HTTP_FORWARDED'))
                                                    ...
                                                    $cek_kasir = $this->db->get_where('transaksi_kasir', array('validasi' => '',
                                                        'kode_kasir' => $nomor_kasir, 'petugas' => $nama_petugas));
...
public function get_produk_manual()
{
    ...
    public function jenis_produk(){
        $data = $this->input->post();
        $produk = $this->db->get_where('master_bahan_baku',array('kode_bahan_baku'=>$data['id_menu']));
        $hasil_produk = $produk->row();
        if($hasil_produk->jenis_produk=="Regular"){
            echo '<input readonly="true" type="text" name="harga" id="harga" class="form-control" placeholder="harga">';
        }else{
            echo '<input type="text" name="harga" id="harga" class="form-control" placeholder="harga">';
        }
    }
    ...
    public function simpan_pesanan_temp()
    {
        ...
        if (count($hasil_cek_menu) < 1) {
            ...
            if($hasil_getmenu->real_stock<$masukan['jumlah']){
                $cek='1';
            } else{
            ...
            $updatemenu['hpp'] = $hasil_getmenu->hpp;
            $updatemenu['kode_satuan'] = $hasil_getmenu->id_satuan_stok;
            ...
            if($hasil_getmenu->real_stock<$updatemenu['jumlah']){
                $cek='1';
            } else{
                $this->db->update('opsi_transaksi_penjualan_temp', $updatemenu, array(
                    'kode_penjualan' => $masukan['kode_penjualan'],
                    'kode_menu' => $kode_produk,
                    'kode_satuan' => $hasil_getmenu->id_satuan_stok));
            }
        }
        if($cek=='1'){
            echo "gagal";
    ...
    public function simpan_pembayaran()
    {
        ...
        foreach ($hasil_pesanan as $daftar) {
            ...
            $stok = $this->db->get_where('master_bahan_baku', array('kode_bahan_baku' => $daftar->
                kode_menu));
            $hasil_stok = $stok->row();

            $updatestok['real_stock'] = $hasil_stok->real_stock - ($daftar->jumlah);
            $this->db->update('master_bahan_baku', $updatestok, array('kode_bahan_baku' => $daftar->
                kode_menu));

            $trans_stok['jenis_transaksi'] = 'penjualan';
            $trans_stok['kode_transaksi'] = $data['kode_penjualan_baru'];
            $trans_stok['kategori_bahan'] = $daftar->kategori_menu;
            $trans_stok['kode_bahan'] = $daftar->kode_menu;
            $trans_stok['nama_bahan'] = $daftar->nama_menu;
            $trans_stok['stok_keluar'] = $daftar->jumlah;
            $trans_stok['hpp'] = $daftar->hpp;
        ...
        $transaksi['kode_transaksi'] = @$hasil_kode_penjualan->kode_penjualan."_".date("dmyHis")."_".$no_belakang;
        $transaksi['urut'] = @$no;
        ...
        #echo $this->db->last_query();
        if ($data['jenis_penerimaan'] == "diambil") {
        ...
        } elseif ($data['jenis_penerimaan'] == "dikirim") {
            $keuangan['kode_jenis_keuangan'] = '1';
            $keuangan['nama_jenis_keuangan'] = 'Pemasukan';
            $keuangan['kode_kategori_keuangan'] = '1.1';
            $keuangan['nama_kategori_keuangan'] = 'Penjualan';
            ...
            if ($data['jenis_transaksi'] != 'cod') {
                ...
                if ($data['jenis_transaksi'] == 'kredit') {
                    $piutang['kode_transaksi'] = $data['kode_penjualan_baru'];
                    //$piutang['kode_piutang'] = $hasil_kode_ro->kode_penjualan."_".$code;
                    $piutang['kode_customer'] = $data['kode_member'];
                    $piutang['nama_customer'] = $data['nama_member'];
                    $piutang['nominal_piutang'] = $data['grand_total'] - $data['bayar'];
                    $piutang['sisa'] = $data['grand_total'] - $data['bayar'];
    ...
    public function get_taking_order()
    {
        ...
        foreach ($hasil_to as $daftar) {
            $this->db->where('kode_bahan_baku', $daftar->kode_menu);
            $get_barang = $this->db->get('master_bahan_baku');
            $barang = $get_barang->row();
            ...
            $penjualan_temp['kode_satuan'] = $barang->id_satuan_stok;
            $penjualan_temp['nama_satuan'] = $barang->satuan_stok;
    ...
    public function cetak_pembayaran()
    {
        ...
        $member = $this->db->get_where('transaksi_penjualan', array('kode_penjualan' => $kode_penjualan));
        $hasil_member = $member->row();
        ...
        }else if ($hasil_member->kode_member == '') {

            $printTestText .= "Kode Member  : -                                 \n";
            $printTestText .= "Nama Member  : -                                 \n";
...
public function cetak_pembayaran_personal()
{
    ...
    {
        $kode = $this->input->post('query');
        $get_member = $this->db->query('SELECT * FROM master_member WHERE kode_member LIKE "%'.$kode.'%"');
        $hasil_member = $get_member->result_array();
        $json = array();
        foreach($hasil_member as $row){
         $json[] = $row['kode_member'];
     }

     echo json_encode($json);
 }
 public function get_pesanan_temp_hapus()
 {
    $kode = $this->input->post('query');
    $kode_kasir = $this->input->post('kode_kasir');
    $get_menu = $this->db->get_where('opsi_transaksi_penjualan_temp',array('kode_kasir'=>@$kode_kasir));
