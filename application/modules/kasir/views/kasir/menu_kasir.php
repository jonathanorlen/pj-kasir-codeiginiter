        ...
        <div class="portlet box grey-steel">
          <div class="portlet-title">
            ...
            <div class="pull-right">   
             <a onclick="kasir_manual()" id="kasir_manual" style="text-decoration: none;font-size: 16px;" class="btn green btn-md"><i class="fa fa-money"></i> Kasir Manual</a>
          ...
          <td width="250px" style="background-color:#229fcd;">
            ...
            <select name="menu" id="menu" class="form-control select2">
              ...
              foreach($hasil_menu as $daftar){
                ?>
                <option value="<?php echo $daftar->kode_bahan_baku; ?>"><?php echo $daftar->nama_bahan_baku.' - '.format_rupiah($daftar->harga_jual_1); ?></option>
                <?php } ?>
              </select>
...
<div id="j_produk">
<input placeholder="ket" type="text" class="form-control" id="warna"; />
  ...
  <div style="">
    <div class="form-group">
      <div class="input-group">
        ...
        <span>
          <input type="text" class="form-control" id="kode_member" name="kode_member" autocomplete="off" onchange="get_member()"/>
    ...
    <div style="height:60px; margin-bottom:5px;margin-top: -20px;" class="jatuh_tempo">
      <div style="height: 40px;" class="form-group">
        <div class="input-group">
          ...
          <span id="golongan">
            <input placeholder="Tanggal" type="date" class="form-control" id="tgl_jatuh_tempo" value="<?php echo date('Y-m-d') ?>"/>
          </span>
        </div>
      </div>
    </div>
  ...
  <fieldset>
    ...
    <div class="col-md-4">
      ...
      <div class="form-group">
        ...
        <input placeholder="Tanggal" type="date" class="form-control" id="tgl_kirim" value="<?php echo date('Y-m-d') ?>"/>
      </div>
...
<div class="row">
  ...
  <div class="col-lg-12">
    <br>
    <br>
    <div style='font-size: 1.5em;' class='note note-bordered note-warning'>
      <h3><b>Shortcut :</b></h3>
      <ul>
        <li><b>ENTER</b> --- Untuk Fokus ke Form Selanjutnya</li>
        <li><b>Esc</b> --- Untuk Mengakhiri Transaksi / Pembayaran</li>
        <li><b>Ctrl</b> --- Untuk Membuat / Menambah Opsi Penjualan</li>
        <li><b>Delete</b> --- Untuk Menghapus Opsi Penjualan</li>
        <li><b>End</b> --- Untuk Menutup Form/Pop Up Hapus Opsi Penjualan</li>
        <li><b>Spasi</b> --- Untuk Memilih Produk dan Memilih Jenis Diskon Opsi Penjualan</li>
      </ul>
    </div>
  </div>
</div><br /><br />
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
       ...
       <?php
       $this->db->where('kategori_kasir','Manual');
       $menu_resto = $this->db->get('master_bahan_baku');
       $hasil_menu = $menu_resto->result();
       ?>
...
<script>
  ...
  function simpan_manual(){
    ...
    var menu = $("#nama_produk_manual").val();
    var jumlah = $("#qty_manual").val();
    var harga = $("#harga_manual").val();
    var diskon = '0';
    var kode_kasir = $("#kode_kasir").val();
    var jenis_diskon = '';
    var diskon_rupiah = '0';
    if(jumlah ==0 || menu==""){
      $(".sukses_manual").html("<div class='alert alert-warning'>Jumlah Pesanan Salah</div>");
      setTimeout(function(){$('.sukses_manual').html('');},1500); 
      ...
      $.ajax( {
        ...
        success : function(data) {
          if(data.length==7){
            ...
            $(".tunggu").hide(); 
            $("#tb_pesan_temp").load('<?php echo base_url().'kasir/kasir/pesanan_temp/'; ?>'+kode_kasir);
            $("#nama_produk_manual").select2().select2('val', '');
            $("#qty_manual").val('');
            $("#diskon_item").val('0');
            $("#harga_manual").val('');
            $("#keterangan").val('');
...
function bayar(){
  ...
  var ongkos = $("#ongkos_kirim").val();
  var jatuh_tempo = $("#tgl_jatuh_tempo").val();
    ...
    if(nama_member=="" && jenis_transaksi=="kredit"){
      ...
      $.ajax({
        ...
        data: {kode_meja:kode_meja,kode_penjualan:kode_penjualan,bayar:bayar,kembalian:kembalian,
          ...
          jam_kirim:jam_kirim,tgl_kirim:tgl_kirim,ongkos:ongkos,jatuh_tempo:jatuh_tempo},
...
$(document).ready(function(){
  ...
  function jenis_produk(){
    var id_menu = $("#menu").val();
    var url = "<?php echo base_url().'kasir/jenis_produk'; ?>";
    $.ajax( {
      type:"POST", 
      url : url,  
      cache :false,
  ...
  $('#barang_hapus').on('keydown', function(event) {
    if (event.which == 35) {
      $("#modal-hapus-barang").modal('hide');
    }
    if (event.which == 32) {
      $("#barang_hapus").select2('show');
    }
  });
  $('#barang_hapus').on('keydown', function(event) {
    if (event.which == 13) {
...
$("#modal-hapus-barang").on('shown.bs.modal', function () {
  $("#barang_hapus").focus();
  get_select_temp()
});
...
$('#kode_member').typeahead({
  source:  function (query, process) {
    var textVal=$("#kode_member").val();
    $.ajax({
      url: '<?php echo base_url().'kasir/get_kode_member'; ?>',
      type: 'POST',
      data: {query:query},
      dataType: 'JSON',
      async: true,
...
function get_select_temp(){
  var kode_kasir = $("#kode_kasir").val();
  $.ajax({
    url: '<?php echo base_url().'kasir/get_pesanan_temp_hapus'; ?>',
    type: 'POST',
    data: {kode_kasir:kode_kasir},
    success: function(data) {
      $('#barang_hapus').html('');
      $('#barang_hapus').html(data);
      $("#barang_hapus").select2().select2('val', '');
    }
  });
}
$('.form-control').focus(function(){
  $(this).css({'background-color' : '#91d096'});
  $(this).css({'border-color' : '#2ab567'});
});
