...
class kasir extends MY_Controller
{
    ...
    public function buka_kasir()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (empty($hasil_cek->kode_transaksi)) {
                ...
                echo "<div style='font-size: 1.5em;' class='alert alert-warning'>Kasir Belum Divalidasi</div>";
            }
        }
 ...
 public function laporan()
    ...
    public function simpan_pesanan_temp()
    {
		...
		'status_meja'=>'digabung'));
        $hasil_cek_gabung = $cek_gabung->result();
        $meja_gabung = $cek_gabung->row();
        
       	$jual = $this->db->get_where('opsi_transaksi_penjualan_temp',array('kode_penjualan'=>@$hasil->kode_penjualan,
		'status_meja'=>'digabung','kode_meja'=>$kode_meja));
        $hasil_jual = $jual->result();
        
        $jml_jual = count($hasil_jual);
        
        echo "HASIL".count($hasil_cek_gabung);
        if (count($hasil_cek_gabung) < 1) {
            ...
            if (count($hasil_cek_menu) < 1) {
                ...
                $updatemenu['kategori_menu'] = $hasil_getmenu->kategori_menu;
                $updatemenu['nama_menu'] = $hasil_getmenu->nama_menu;
                $updatemenu['kode_satuan'] = $hasil_getmenu->kode_satuan_stok;
                $updatemenu['nama_satuan'] = $hasil_getmenu->satuan_stok;
                $updatemenu['jumlah'] = $masukan['jumlah'] + $hasil_cek_menu->jumlah;
                $harga_diskon = ($masukan['harga_satuan'] * $masukan['diskon_item']) / 100;
                $updatemenu['harga_satuan'] = $masukan['harga_satuan'] - $harga_diskon;
                $updatemenu['diskon_item'] = $masukan['diskon_item'];
                $subtotal = ($masukan['jumlah'] + $hasil_cek_menu->jumlah) * $masukan['harga_satuan'];
                $updatemenu['subtotal'] = $subtotal;
                $updatemenu['status_menu'] = $hasil_getmenu->status_menu;

                $this->db->update('opsi_transaksi_penjualan_temp', $updatemenu, array(
                    'kode_penjualan' => $masukan['kode_penjualan'],
                    'kode_meja' => $kode_meja,
                    'kode_menu' => $kode_menu));
            }
            ...
            $cek_menu = $this->db->get_where('opsi_transaksi_penjualan_temp', array(
                'kode_penjualan' => $meja_gabung->kode_penjualan,
                'kode_meja' => $kode_meja,
                'kode_menu' => $kode_menu));
            $hasil_cek_menu = $cek_menu->row();
            echo count($hasil_getmenu) ;
            if (count($hasil_cek_menu) < 1) {
                
        
                foreach($hasil_cek_gabung as $daftar){
                    $digabung['kode_kasir'] = $daftar->kode_kasir;
                    ...
                    $digabung['status_meja'] = 'digabung';
                    $this->db->insert('opsi_transaksi_penjualan_temp', $digabung);
                }
                
            } else {
                $updategabung['kode_kasir'] = $meja_gabung->kode_kasir;
                $updategabung['kode_penjualan'] = $meja_gabung->kode_penjualan;
                $updategabung['kategori_menu'] = $hasil_getmenu->kategori_menu;
                $updategabung['nama_menu'] = $hasil_getmenu->nama_menu;
                $updategabung['kode_satuan'] = $hasil_getmenu->kode_satuan_stok;
                $updategabung['nama_satuan'] = $hasil_getmenu->satuan_stok;
                $updategabung['jumlah'] = $masukan['jumlah'] + $hasil_cek_menu->jumlah;
                $harga_diskon = ($masukan['harga_satuan'] * $masukan['diskon_item']) / 100;
                $updategabung['harga_satuan'] = $masukan['harga_satuan'] - $harga_diskon;
                $updategabung['diskon_item'] = $masukan['diskon_item'];
                $subtotal = ($masukan['jumlah'] + $hasil_cek_menu->jumlah) * $masukan['harga_satuan'];
                $updategabung['subtotal'] = $subtotal;
                $updategabung['status_menu'] = $hasil_getmenu->status_menu;
                $updategabung['status_meja'] = 'digabung';
                $this->db->update('opsi_transaksi_penjualan_temp', $updategabung, array('kode_penjualan' =>
                        $masukan['kode_penjualan'], 'kode_menu' => $kode_menu));
            }
            ...
            $masukan['kode_penjualan'], 'kode_meja' => $kode_meja,'kode_menu'=>$kode_menu));
            $hasil_cek_menu = $cek_menu->result();
            $digabung['kode_kasir'] = $daftar->kode_kasir;
            $digabung['kode_penjualan'] = $daftar->kode_penjualan;
            $digabung['kode_meja'] = $daftar->kode_meja;
            $digabung['kode_menu'] = $hasil_getmenu->kode_menu;
            $digabung['jumlah'] = $masukan['jumlah'] + $daftar->jumlah;
            ...
            $digabung['status_menu'] = $hasil_getmenu->status_menu;
            #  echo $daftar->jumlah;
            echo count($hasil_cek_menu);
            if (count($hasil_cek_menu) < 1) {
            $this->db->insert('opsi_transaksi_penjualan_temp', $digabung);
            }else{

            $this->db->update('opsi_transaksi_penjualan_temp',$digabung,array('kode_penjualan'=>$daftar->kode_penjualan,'kode_menu'=>$hasil_getmenu->kode_menu));
            }
    ...
    public function simpan_pembayaran()
    {
        ...
        $cek_gabung = $this->db->get_where('opsi_transaksi_penjualan_temp', array('kode_meja' =>
                $kode_meja));
        $hasil_cek = $cek_gabung->row();
        #$this->db->group_by('kode_meja');
        
        
        $jual = $this->db->get_where('opsi_transaksi_penjualan_temp',array('kode_penjualan'=>@$hasil_cek->kode_penjualan,
		'status_meja'=>'digabung','kode_meja'=>$kode_meja));
    	$hasil_jual = $jual->result();
        #echo $this->db->last_query();
    	$jml_jual = count($hasil_jual);
        	...
        	if($jml_jual >=1){
                ...
                $hasil_cek->kode_penjualan,'status_meja'=>'digabung'));
                $hasil_pesanan = $get_pesanan->result();
        	}else{
        	   $get_pesanan = $this->db->get_where('opsi_transaksi_penjualan_temp', array('kode_penjualan' =>
                $hasil_cek->kode_penjualan,'kode_meja'=>$kode_meja));
                $hasil_pesanan = $get_pesanan->result();
        	}
        ...
        $get_pesanan_transaksi = $this->db->get_where('opsi_transaksi_penjualan_temp',
            array('kode_penjualan' => $hasil_cek->kode_penjualan,'kode_meja'=>$kode_meja));
        ...
        foreach ($hasil_pesanan_transaksi as $simpan) {
            ...
            $transaksi['kode_kasir'] = $simpan->kode_kasir;
        	...
        	if($jml_jual >=1){
        	    $this->db->delete('opsi_transaksi_penjualan_temp', array('kode_penjualan' =>$hasil_cek->kode_penjualan,'status_meja'=>'digabung'));
        
        	}else{
        	   $this->db->delete('opsi_transaksi_penjualan_temp', array('kode_penjualan' =>$hasil_cek->kode_penjualan,'kode_meja'=>$kode_meja));
    ...
    public function hapus_pesanan_temp()
    {
        ...
        $cek_gabung = $this->db->get_where('opsi_transaksi_penjualan_temp', array('kode_menu' =>
                $hasil_cek->kode_menu, 'kode_penjualan' => $hasil_cek->kode_penjualan));
        $hasil_cek_gabung = $cek_gabung->result();

        foreach ($hasil_cek_gabung as $hapus) {
            $cek_menu = $this->db->get_where('master_menu', array('kode_menu' => $hapus->
                    kode_menu));
            $hasil_cek_menu = $cek_menu->row();
            if ($hasil_cek_menu->status_menu == "reguler") {

            } else
                if ($hasil_cek_menu->status_menu == "tambahan") {

                }

            $this->db->delete('opsi_transaksi_penjualan_temp', array('kode_menu' => $hapus->
                    kode_menu, 'kode_penjualan' => $hapus->kode_penjualan));
        }
    ...
    public function gabung_meja()
    {
        ...
        $this->db->update('opsi_transaksi_penjualan_temp',$update_status,array('kode_meja'=>$kode_meja));
        $hasil_cek = $cek_meja->result();
    ...
    public function cetak_pembayaran()
    {
        ...
        $menu = $this->db->get_where('opsi_transaksi_penjualan', array('kode_meja' => $kode_meja,
        ...
        $printTestText .= "$hasil  	: " . format_rupiah(abs($hasil_penjualan->kembalian)) .
            "\n";
        $printTestText .= "---------------------------------------------\n";
        $printTestText .= "                   Terima Kasih             \n";
        $printTestText .= "        Barang yang sudah dibeli        \n";
